...
import { Users, Gift, Award, Lock } from "lucide-react";
...
const ReferralDashboard = () => {
  ...
  const calculateProgressToNextTier = () => {
    ...
    // Find current tier's required referrals
    const currentTier = referralInfo.tiers.find(
      (tier) => tier.requiredReferrals > currentReferrals
    );
    
    if (!currentTier) return 100; // All tiers completed
    
    const previousTierReferrals = currentTier.level > 1
      ? referralInfo.tiers[currentTier.level - 2].requiredReferrals
      : 0;
      
    const tierProgress = ((currentReferrals - previousTierReferrals) /
      (currentTier.requiredReferrals - previousTierReferrals)) * 100;
            ...
            <>
              <div className="space-y-2">
                <div className="flex justify-between text-sm">
                  <span>
                    {referralInfo?.referralsCount || 0} Shadows Recruited
                  </span>
                  <span>
                    Next Milestone: {referralStats?.nextMilestone || 5} Referrals
                  </span>
                </div>
                <Progress value={calculateProgressToNextTier()} />
              </div>
